 ...
 */
final class UrlShortenerTableGateway
    extends TableGateway
    implements UrlShortenerPersistenceInterface
{
    /**
    ...
    {
        $rowSet = $this->select(function (Select $select) use ($shortUrl) {
            $select
                ->columns(['long'])
    ...
    public function persistUrl(string $longUrl, string $shortenedUrl): bool
    {
        ...
        return (bool)$this->insertWith($insert);
    }
}